...
function renderMinorsTable() {
  ...
  state.minors.forEach((co, idx) => {
    ...
    const _placeLabel = co.homeHex
      ? (co.homeCityIndex !== undefined ? `${co.homeHex}·${co.homeCityIndex}` : co.homeHex)
      : 'Place ⊕';
    const placeBtn = co.locationMechanism === 'fixed' ? `<button class="table-btn place-btn" data-idx="${idx}" style="background:#444;margin-top:4px;font-size:10px;">${_placeLabel}</button>` : '<span style="color:#666;font-size:10px;">-</span>';

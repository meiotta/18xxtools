...
PRR home city. One city plus a bypass. Two exits (left and right in pointy-top 1830 grid) forming a straight chord. Two rendered features: a straight bypass line between the two exits, and a wide shallow circular arc from left exit through the city (positioned ~28 units above center) to right exit. The circumcircle of the three points (left exit, city, right exit) has its center below the hex. At circumradius=100: arc center ≈ 119.9 units below center, arc radius ≈ 147.8 units, sweep ≈ 66°. At circumradius=50 (our system): divide all by 2.

Tobymao DSL: `path=a:4,b:_0; path=a:1,b:_0; path=a:4,b:1` where `_0` is the city node.
